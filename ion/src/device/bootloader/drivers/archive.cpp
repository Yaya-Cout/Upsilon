...
bool isSane(const TarHeader *tar) {
  return !memcmp(tar->magic, "ustar  ", 8) && tar->name[0] != '\x00' &&
         tar->name[0] != '\xFF';
}
...
bool fileAtIndex(size_t index, File &entry) {
  ...
  for (;;) {
    ...
    if (index == 0) {
      ...
      stride = (stride >> 9) << 9;
      tar = reinterpret_cast<const TarHeader *>(
          reinterpret_cast<const char *>(tar) + stride);
...
typedef uint32_t (*entrypoint)(const uint32_t, const void *, void *,
                               const uint32_t);
